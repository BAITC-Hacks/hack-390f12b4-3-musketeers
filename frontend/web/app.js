...
function mapSvg() {
  return '<div class="map-wrap"><svg class="city-map" viewBox="0 0 600 405" aria-label="Условная интерактивная схема пяти районов"><defs><pattern id="mapgrid" width="24" height="24" patternUnits="userSpaceOnUse"><circle cx="1" cy="1" r=".65" fill="#dde8e2"/></pattern></defs><rect width="600" height="405" fill="url(#mapgrid)"/><path d="M-15 174C98 194 111 235 199 199S299 134 346 172 389 169 431 171 478 272 625 265" stroke="#d1e6df" stroke-width="13" fill="none"/><path d="M-15 174C98 194 111 235 199 199S299 134 346 172 389 169 431 171 478 272 625 265" stroke="#fff" stroke-opacity=".7" stroke-width="1.5" fill="none"/>' +
    state.projection.districts.map(d => {
      const shape = mapShapes[d.id], score = state.mapMode === "before" ? d.score_before : d.score_after;
      const color = score < 50 ? "#f1e8d7" : score < 58 ? "#dbe9e4" : score < 65 ? "#cde2dc" : "#b9d8cf";
      return '<g><path class="map-district ' + (state.district === d.id ? 'selected' : '') + '" d="' + shape.d + '" fill="' + (state.district === d.id ? "#d8ebe2" : color) + '" data-district="' + d.id + '" role="button" tabindex="0" aria-label="Район ' + escape(d.name) + ', оценка ' + number(score) + '" aria-pressed="' + (state.district === d.id) + '"/><text class="map-label ' + (state.district === d.id ? 'selected-label' : '') + '" x="' + shape.x + '" y="' + shape.y + '">' + escape(d.name) + '</text><text class="map-label map-score" x="' + shape.x + '" y="' + (shape.y+22) + '">' + number(score,1) + ' балла</text></g>';
...
function districtDetail() {
  ...
  return '<section class="panel district-detail"><div class="panel-body"><div class="detail-main"><div class="row between"><span class="eyebrow">РАЙОН В ФОКУСЕ</span><span class="pill ' + (critical.length ? "amber" : "green") + '">' + (critical.length ? "Нужна поддержка" : "Без критических значений") + '</span></div><div class="row between"><h2 class="district-title">' + escape(d.name) + '</h2><span class="district-number">' + number(state.mapMode === "before" ? d.score_before : d.score_after,1) + '</span></div><p class="district-profile">Исходный профиль: ' + escape(d.profile) + '</p></div><div class="detail-meters">' +
...
function cityView() {
  return heading("ГОРОДСКАЯ ЛАБОРАТОРИЯ · АСТАНА","Решения меняют город","Пять районов, пять решений и один общий бюджет. Посмотрите, где помощь нужнее всего.",
    button(icon("spark") + "Открыть пример","example")) + stats() +
    ...
    mapSvg() + '<div class="map-legend"><span class="legend-item"><i class="dot amber"></i>Оценка ниже 50</span><span class="legend-item"><i class="dot blue"></i>Оценка от 50</span><span class="legend-last">Больше — лучше</span></div></section>' + districtDetail() + '</div>' + eventPanel() +
...
function planPanel() {
  ...
  return '<aside class="panel plan-panel"><div class="panel-body"><div class="plan-head"><div><span class="eyebrow">ШАГ 2 · ВАШ ВЫБОР</span><h2>Ваш план</h2></div><span class="pill neutral">' + state.plan.length + ' / 5</span></div><p class="plan-guidance">' + (state.plan.length === 5 ? 'Пять решений выбраны. Можно оценить результат или заменить любую меру ниже.' : 'Добавьте ' + (5-state.plan.length) + ' ' + (5-state.plan.length === 1 ? 'меру' : 5-state.plan.length === 5 ? 'мер' : 'меры') + '. Суммарная стоимость не должна превысить 100 ед.') + '</p>' +
...
function catalogView() {
  ...
  return heading("ШАГ 1 ИЗ 2 · СОБЕРИТЕ ПЛАН",state.plan.length === 5 ? "Пять решений выбраны" : "Выберите пять решений","Нажмите на меру, чтобы увидеть её цену, эффект и доступные районы. Затем добавьте её в план.",
    button(state.plan.length === 5 ? "Посмотреть результат " + icon("arrow") : icon("spark") + "Готовый пример",state.plan.length === 5 ? "calculate" : "example",state.plan.length === 5 ? "primary" : "")) + stats() +
    '<div class="catalog-guide" aria-label="Как собрать план"><div><b>01</b><span>Выберите направление<br><small>или смотрите все меры</small></span></div><div><b>02</b><span>Откройте меру<br><small>проверьте цену и эффект</small></span></div><div><b>03</b><span>Добавьте в план<br><small>ровно 5 мер в пределах 100 ед.</small></span></div></div>' +
    ...
    '<div class="catalog-section-head"><div><span class="eyebrow">КАТАЛОГ МЕР</span><h2>Что можно сделать</h2></div><span>' + filtered.length + ' ' + (filtered.length === 1 ? 'инициатива' : filtered.length >= 2 && filtered.length <= 4 ? 'инициативы' : 'инициатив') + '</span></div><div class="catalog-tools" aria-label="Фильтры инициатив">' + [["all","Все инициативы"],...Object.entries(DIR)].map(([key,title]) => '<button class="filter ' + (state.filter === key ? 'active' : '') + '" data-filter="' + key + '" aria-pressed="' + (state.filter === key) + '">' + (key === "all" ? '' : icon(key)) + title + '</button>').join("") + '</div><div class="content-grid catalog-content"><div class="catalog-grid">' +
    filtered.map(m => {
      const added = state.plan.some((d,i) => d.measure_id === m.id && i !== state.edit);
      return '<article class="initiative"><div class="row between">' + categoryIcon(m.direction) + '<span class="pill ' + (added ? 'green' : 'neutral') + '">' + (added ? "В плане" : m.scope === "city" ? "Весь город" : "Один район") + '</span></div><span class="initiative-direction">' + escape(DIR[m.direction]) + '</span><h3>' + escape(m.title) + '</h3><p>' + escape(DESCRIPTIONS[m.id]) + '</p><div class="initiative-meta"><span>' + icon("clock") + 'Эффект через ' + m.lag + ' кв.</span><span>' + icon("pin") + (m.scope === "city" ? "5 районов" : "Район на выбор") + '</span></div><div class="initiative-footer"><span class="price">' + m.cost + ' <small>ед. из бюджета</small></span>' + button(added ? icon("check") + "В плане" : "Посмотреть эффект " + icon("arrow"),"measure",added ? "small" : "soft small",'data-measure="' + m.id + '" ' + (added ? 'disabled' : '')) + '</div></article>';
    }).join("") + '</div>' + planPanel() + '</div><details class="catalog-extra"><summary>Дополнительно: изменить условия города</summary>' + eventPanel() + '</details><div class="mobile-plan"><span>План: ' + state.plan.length + ' / 5 · ' + state.projection.spent + ' ед.</span>' + button(state.plan.length === 5 ? "Оценить план" : "Открыть мой план", state.plan.length === 5 ? "calculate" : "show-plan","primary small") + '</div>';
}
...
function resultsView() {
  ...
  return html + '<div class="result-layout"><section class="score-hero"><div><span class="eyebrow">ASTANA QUALITY OF LIFE SCORE</span><h2>' + (gain > 0 ? "Город стал комфортнее" : gain < 0 ? "План требует пересмотра" : "Оценка города не изменилась") + '</h2><p>Бюджет — ' + r.spent + ' из 100. Критических значений: ' + r.result.critical_count + ' вместо ' + r.baseline.critical_count + '.</p></div><div><div class="score-big">' + number(r.result.score) + '</div><div class="score-delta">' + signed(gain) + ' к исходным ' + number(r.baseline.score) + '</div></div></section><div class="result-columns"><div class="stack"><div class="decomposition">' +
    ...
    recommendationsView() + '</div><aside class="result-actions" aria-label="Дальнейшие действия"><section class="result-next"><span class="eyebrow">СЛЕДУЮЩИЙ ШАГ</span><span class="result-next-icon">' + icon("spark") + '</span><h2>Почему получился этот балл?</h2><p>AI разберёт ваши пять решений: что улучшилось, где остаются риски и как усилить план.</p>' +
    button("Перейти к AI-анализу " + icon("arrow"),"analysis","primary full") + '</section><section class="result-save"><span class="eyebrow">ДЛЯ СРАВНЕНИЯ</span><h2>Сохранить план</h2><p>Дайте сценарию имя, чтобы сравнить его с другим вариантом.</p><label for="scenario-name">Название сценария</label><input id="scenario-name" type="text" maxlength="60" placeholder="Например, «Забота о районах»">' +
    button(icon("compare") + "Сохранить сценарий","save","full") + '<div class="result-export">' + button(icon("download") + "Скачать JSON","export","ghost") + '</div><p class="subtle-note">До 4 сценариев сохраняются в этом браузере.</p></section></aside></div></div>';
}
